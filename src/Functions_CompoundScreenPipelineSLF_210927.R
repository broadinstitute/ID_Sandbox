#9/27/21
#Edit: add removal of DMSO wells with low counts prior to SLF calculation

library(dplyr)
library(tidyr)
library(stringr)
# library("cmapR", lib.loc="/home/unix/jbagnall/R/x86_64-pc-linux-gnu-library/3.2")
...
compScreenPipeline <- function(rawcounts_df, untreatedname = "untreated", intconname = "control", comp_conc_separator = ":", lowwellcount = 10, low_untreated_count = 100, medianLFC = FALSE, newSchema = F, savefilename = NA, saveCSV = F){ 
  #190917 Rewriting log fraction pipeline
  ...
  #"corrected" log fraction (normalized by plate and untreated), standardized residuals (Michael's "normed")
  #p-value, replicate correlation, rounded concentration or concentration rank?
  ...
  # rawcounts_subset <- mutate(rawcounts_subset, well_pos = paste(row, str_pad(as.character(column), 2,side = "left", "0"), sep = ""))
  # rawcounts_subset <- distinct(rawcounts_subset)
  ...
  #Moved up 9/27/21: remove wells with technical error
  #Remove all wells where intcon counts are 0--added 9/17/19
  ...
  #9/27/21 add wellcount removing spike-in controls
  ...
  #Added 2/24/21 Save which wells were removed due to low total counts
  #Edit 9/27/21 Add DMSO wells with low count (different total well count threshold) to the "removed" list
  ...
  #Remove plates with no untreated information (could have been removed due to low counts or no intcon)--moved down on 8/4/21
  #Edit 9/27/21: Since we removed untreated wells with low counts, now make the criteria to have 3 or more "good' wells per plate, otherwise remove.
  ...
  #Remove plates where there are < 3 untreated wells that have good counts 
  #Edit 9/27/21. Since we now remove DMSO wells with low counts, as assumed technical error, then don't need to recalculate wellcount. Wrap removal of plate into code above
  # untreated_plates <- untreated_raw %>%
  #   group_by(plate_name, row, column) %>%
  #   summarise(wellcount = sum(count, na.rm = T), goodwellcount = wellcount > 10) %>% #some what arbitrary small number
  #   ungroup()%>%
  #   group_by(plate_name) %>%
  #   summarise(goodwellnum = sum(goodwellcount, na.rm = T)) %>% 
  #   ungroup() %>%
  #   filter(goodwellnum < 3) #Need 3 or more untreated wells otherwise remove plate 
  # 
  # print("Remove plates with < 3 good untreated wells:")
  # print(paste(untreated_plates$plate_name, collapse = "; "))
  # rawcounts_subset <- dplyr::filter(rawcounts_subset, !(plate_name %in% untreated_plates$plate_name))
  # remove(untreated_raw, untreated_sumstrain, untreated_plates)
  
  
  #The location of the following chunk was moved down on 11/25/19
  ...
  if(any(temp$sumcount == 0)){
    ...
    # saveRDS(nountreated, file = paste("/home/unix/jbagnall/GatesProject/StrainPlate_nountreated", format(Sys.time(), "%m_%d_%Y_%s"), ".rds", sep = ""))
    saveRDS(nountreated, file = paste(gsub(".rds", "", savefilename), "_StrainPlate_nountreated", format(Sys.time(), "%m_%d_%Y_%s"), ".rds", sep = ""))
    print(nountreated)
  }
  
  
  #11/25/19 Moved replicate_id assignment to after removing all anomolous wells
  ...
  #Need to consider what to do for plates that have no "untreated" wells.
  ...
  if(!is.na(savefilename)){
    saveRDS(rawcounts_subset, file = savefilename) #Saves df as rds file in specified path
    if(saveCSV){
...
replicatecorrelation <- function(screendata, metric = "std_lf", comp_conc_separator = ":", randomize = FALSE, removemin = FALSE, savefilename = NA){
  #3/21/19 Adds a loop to go through all replicates (rather than just the first 2) 
  #9/17/19 Returns NA for correlation if there is no replicate
  #Calculate Pearson correlation between Rep_0 and Rep_1. Averages multiples if present.
  #Input
  ...
  #########optional add-ins to revamp if needed ####
  #   if(randomize){
  #     #Next time reset this to do it on rep_0 and rep_1 only
  #     #Randomize rep1 column
  #     #This can serve as a null distribution of replicate correlations
  #     combinerep_all$rep1_rand <- base::sample(combinerep_all$rep_1)
  #     combinerep_rand <- combinerep_all %>%
  #       group_by(comp_conc) %>%
  #       summarise(correlation_rand = cor(rep_0, rep1_rand))
  #     hist(combinerep_rand$correlation_rand, 50, main = "Randomized replicate correlation", xlab = "Replicate correlation (Pearson)")
  #     combinerep <- full_join(combinerep, combinerep_rand, by = "comp_conc")
  #   }
  
  #   if(removemin){ #maybe turn this into another function to do this iteratively
  #     #Remove minimum and recalculate Pearson correlations to see if there is the case where one strain drives the correlation strongly
  #     #Return new correlation values as well as strain that was removed to calculate the new correlation
  #     ##Remove minimum point and recalculate correlations. See how much they change. Is this indicative of the drug 
  #     #only affecting one strain?
  #     screendata_01 <- screendata %>%
  #       dplyr::filter(rep == "rep_0" | rep == "rep_1" ) %>%
  #       group_by(comp_conc, strain, rep) %>%
  #       summarise(avg_std_lf = mean(std_lf)) %>%
  #       ungroup() %>%
  #       group_by(comp_conc) %>%
  #       slice(-which.min(avg_std_lf)) %>%
  #       spread(key = rep, value = avg_std_lf, drop = FALSE)
  #     removedstrains <- screendata_01[!complete.cases(screendata_01),1:2]
  #     screendata_minremoved <- screendata_01[complete.cases(screendata_filt_01),] 
  #     
  #     screendata_minremoved <- screendata_minremoved %>%
  #       group_by(comp_conc) %>%
  #       summarise(correlation_removemin = cor(rep_0, rep_1))
  #     
  #     combinerep <- full_join(combinerep, screendata_minremoved, by = "comp_conc")
  #     combinerep <- mutate(combinerep, diff = correlation2 - correlation_removemin)
  #     combinerep <- full_join(combinerep, removedstrains, by = "comp_conc")
  #     
  #   }
